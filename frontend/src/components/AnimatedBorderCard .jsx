    ...
    <motion.div
          ...
          className={`relative group rounded-2xl p-[2px] h-full`}>
            ...
            <div className="relative z-10 bg-[var(--popover)] rounded-2xl p-6 border h-full">
              <div className=''>
                <div className="rounded-sm bg-[var(--secondary)] inline-flex justify-center items-center p-2">{icon}</div>
                <h3 className='text-xl text-[var(--foreground)] mb-4 mt-4'>{heading}</h3>
              </div>
              <p className='text-sm text-[var(--primary)]'>{description}</p>
              {other}
            </div>
      </motion.div>
  )
